...
// 🔥 Store Suggested Retail Price BEFORE cost factor
const retailPrice = totalPrice / costFactor; // 🔄 Reverse the cost factor multiplication
...
setSuggestedRetailPrice(retailPrice);
  ...
  useEffect(() => {
    ...
    // ✅ Convert dimensions with fractions
    const widthInches = parseFloat(width) + calculateInches(0, widthFraction);
    const heightInches = parseFloat(height) + calculateInches(0, heightFraction);
    ...
    // ⛔ Skip all validation if Quick Ship Panels
    ...
    console.log(`💰 Final Total Price: $${Math.round(finalTotal)}`);
  }, [
    width,
    height,
    pricingRules,
...
>
  ...
  /* ✅ Render Control Options for Other Products */
  ...
  .filter(([key, value]) =>
    ...
    (optionsData[key]?.includes(value) || sizeBasedPricing[value])
      ...
      .map(([categoryKey, value]) => {
        ...
        const pricingData = sizeBasedPricingData[value]; // ✅ Use correct object
        let optionPrice = 0;
